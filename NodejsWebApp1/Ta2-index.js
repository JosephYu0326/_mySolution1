...
//----------------------------------------------------------------------------------//
//Ta2. 在Express中存取MySQL資料庫 III (以AJAX傳送表單資料)
//Client測試網址:http://localhost:1337/ta2-ins_prod-form.html
app.get("/ta2-insprod", function (req, res, next) {
    var productname = req.query.productname,
        categoryid = req.query.categoryid,       
        unitprice = req.query.unitprice,
        unitsinstock = req.query.unitsinstock;
    var sql =
        `insert into products(productname,categoryid,unitprice,unitsinstock) values(?,?,?,?)`;
    pool.query(sql, [productname,categoryid,unitprice, unitsinstock], function (err, results) {
        if (err) {
           res.send({ result: 'fail', errMsg: err.message });
           //send()會先將「物件參數」轉成JSON格式的字串再送出
        }
        else
            res.send({ result: 'success', rowCount: results.affectedRows });
    });
